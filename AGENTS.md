...
- `uvx --with click --with openai --with anthropic --with httpx --with prompt_toolkit --with PyYAML --with markdownify --with Jinja2 --with rich pyright nano-agent`: run static type checks in a `uvx` environment that matches the script header dependencies.
...
## Testing Guidelines
This repo currently uses built-in self-tests instead of a separate `tests/` package. Add new regression coverage as `self_test_<behavior>()` functions and register each case in `SELF_TEST_CASES`. Keep tests deterministic and offline when possible, especially around provider/network behavior. Before submitting changes, run `./nano-agent self-test`, `uvx ruff check nano-agent`, the `uvx ... pyright nano-agent` command above, and any targeted CLI smoke checks for the command path you touched.
